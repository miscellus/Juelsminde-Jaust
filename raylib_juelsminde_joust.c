...
static float calculate_player_radius(Player *player) {
	return player->params.minimum_radius + player->energy*1.2f;
}


static bool hit_is_hard_enough(float hit) {
...
static float random_01(uint64_t *random_state) {
	...
	val.as_int = (uint32_t)(xorshift64(random_state)&((1 << 23)-1)) | (127 << 23);
...
static void spawn_bullet_ring(Player *player, uint64_t *random_state) {

	uint32_t count = (uint32_t)(player->energy / player->params.bullet_energy_cost_ring);
	...
	player->energy -= (float)count * player->params.bullet_energy_cost_ring;
	...
	for (uint32_t i = 0; i < count; ++i) {
		...
		bullet->time = 0;
		bullet->spin = 15.0f * player->angular_velocity;
		angle += angle_quantum;
	}
...
static void spawn_bullet_fan(Player *player, uint32_t count, float speed, float angle_span) {

	if (player->active_bullets + count > MAX_ACTIVE_BULLETS) {
		count = MAX_ACTIVE_BULLETS - player->active_bullets;
	}

	player->energy -= (float)count * player->params.bullet_energy_cost_fan;
	...
	for (uint32_t i = 0; i < count; ++i) {
		...
		bullet->time = 0;
		bullet->spin = 20.0f * player->angular_velocity;
		angle += angle_quantum;
	}
...
static void spawn_ring(Game_State *game_state, Vector2 position, int player_index, float ring_angle) {
...
static bool position_outside_playzone(Vector2 position, View view) {
	return (
		position.x < -100 ||
...
static void draw_text_shadowed(Font font, const char *text, Vector2 position, float font_size, float font_spacing, Color front_color, Color background_color) {
...
static void reset_game(Game_State *game_state, View view) {
...
#if 0
static void set_window_to_monitor_dimensions(void) {
...
static View get_updated_view(void) {
...
static float shortest_angle_difference(float a, float b) {
...
static int menu_action_toggle_fullscreen(int change, char *user_data) {
	(void)change;
	(void)user_data;
...
static Input get_input_for_player(Player *player) {
...
int main(void)
{
	...
	uint64_t random_state = (uint64_t)time(0);
	...
	while (game_state->running)    // Detect window close button or ESC key
	{
		...
		if (game_state->game_in_progress && !game_state->show_menu && IsWindowFocused()) {
			...
			for (int player_index = 0; player_index < NUM_PLAYERS; ++player_index) {
				...
				if (player->shoot_time_out < game_state->game_play_time) {
					...
					if (input.shoot_action == 1 && player->shoot_charge_t < 1.0f) {
						...
						uint32_t count = (uint32_t)(player->energy / player->params.bullet_energy_cost_fan);
			...
			for (int player_index = 0; player_index < NUM_PLAYERS; ++player_index) {
				...
				// Bounce on view edges
				{
					...
					if (player->velocity.x > 0) {
						...
						if (edge_offset < 0) {
							...
							cumulative_edge_bounce += fabsf(player->velocity.x);
					...
					if (player->velocity.y > 0) {
						...
						if (edge_offset < 0) {
							...
							cumulative_edge_bounce += fabsf(player->velocity.y);
				...
				// Update bullets
				for (uint32_t bullet_index = 0; bullet_index < player->active_bullets; ++bullet_index) {
			...
			int32_t item_change_y = 0;
			int32_t item_change_x = 0;
			int32_t item_enter = IsKeyPressed(KEY_ENTER);
			...
			if (!item_change_y && !item_change_x) {
				...
				if (menu_item_cooldown < current_time) {
					if (item_change_y) {
						menu->selected_index += (uint32_t)((int32_t)menu->item_count + item_change_y);
			...
			switch (item->type) {
				...
				case MENU_ITEM_FLOAT:
					if (item_change_x) {
						*item->u.float_ref += (float)item_change_x * 0.1f;
		...
		ClearBackground((Color){(uint8_t)game_state->color_red, (uint8_t)game_state->color_green, (uint8_t)game_state->color_blue, 255});
		...
		if (game_state->title_alpha > 0) {
			...
			uint8_t title_gray = 160;
			Color title_color = (Color){title_gray, title_gray, title_gray, (uint8_t)(game_state->title_alpha*255.0f)};
		...
		for (int ring_index = 0; ring_index < game_state->active_rings; ++ring_index) {
			...
			float start_angle = ring.angle - 60.0f;
		...
		for (int player_index = 0; player_index < NUM_PLAYERS; ++player_index) {
			...
			if (player_speed <= max_speed_while_drawing_control_text) {
				...
				player->controls_text_timeout = game_state->game_play_time + 5.0f;
			}
		}
		...
		if (triumphant_player >= 0 || game_state->show_menu) {
			...
			DrawRectangle(0, 0, (int)screen.x, (int)screen.y, screen_overlay_color);
			...
			if (!game_state->show_menu) {
				...
				Vector2 draw_position = {0, screen_center.y - 0.5f*colored_background_height};
				...
				float menu_height = (float)menu->item_count * line_advance;
